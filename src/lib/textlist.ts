...
本調査は工学院大学の研究室が実施する金銭分配に関するアンケート調査です。¥n
...
const G_text1 = `1000円の分け方に関する質問を2回お聞きします。¥n
...
ですが、2回目の質問時では1回目の相手と同じ、もしくは異なる相手と分け合う状況を考えていただきます。¥n
本実験はリアルタイムで他の方と同時に実施するわけではありません。
従って、提案者の方は自身の提案が拒否されたかどうかを知ることはありません。¥n
ただ、「提案者」役の方が提示した金額は一旦データベースに保存されたのち、「応答者」役の方に提示されます。
例として1回目の質問で提案者役の方が提案した金額は、他の方が1回目の質問に応答者役として参加した際に提示されます。¥n
1回目の質問で提示した金額が2回目の質問で表示される、もしくはその逆は起こりません。¥n
`;
...
const D_text2 = `調査の事前説明でも述べたとおり、本アンケートの目的は"お金の分配"に関する調査を行うことです。
ですが正確には"初めに提示された金額"が"その後の分配行動"にどのような影響を及ぼすかを調べることが本アンケートの主目的となります。¥n
そのため、1回目の質問時には全ての参加者に応答役が割り当てられており、金額の異なる2通りの分配額が提示されていました。
また、2回目の質問では"その後の分配行動"を調査するため、参加者全員が提案者役に割り当てられています。
従って、実験の事前説明のうち「役割の振り分けは各ゲーム時にランダムに行われる」という点は実際とは異なる説明でした。¥n
1回目のゲームで提示される金額は「250円」と「750円」の2通りしかなく、調査実施者側で設定したものとなっております。
そのため、全ての質問時において、提案者の提示金額が他の参加者に提示されることはございません。¥n
こちらも実験の事前説明で説明したものとは異なっております。¥n`;

const D_text3 = `提示された金額の違いがその後の分配行動にどう影響するか、回答者の自然な反応を調査するために、事実はと異なる説明を行いました。¥n
